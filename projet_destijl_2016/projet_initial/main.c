...
void initStruct(void) {
    ...
    if (err = rt_sem_p(&semGetImage, TM_INFINITE)){ // prise du sémaphore par défaut
        rt_printf("Error semaphore P(): %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&tbattery, &batteryLevel, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_start(&tcamera, &camera_func, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
